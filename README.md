...
> 🧭 **Начни с навигатора:** [`docs/00-index.md`](docs/00-index.md) — карта всех 20 статей, рекомендованный порядок изучения и список примеров Verse.

---

## 🤝 Совместная работа

Репозиторий публичный — друзья могут сразу клонировать:

```bash
git clone https://github.com/AlexAkvalos/uefn-project.git
```

Варианты работы вместе:
- **С правом пуша:** владелец добавляет соавтора — на GitHub: *Settings → Collaborators → Add people* (нужен ник друга), либо `gh repo add-collaborator AlexAkvalos/uefn-project <ник>`.
- **Без прав (через форк):** друг делает **Fork** → правит у себя → присылает **Pull Request**.
- **Перед работой:** `git pull` (забрать чужие изменения), после — `git add . && git commit -m "..." && git push`.

Новые статьи и примеры добавляются по инструкции в конце [`docs/00-index.md`](docs/00-index.md).
...
└── verse/                     ← исходный Verse-код
    ...
    │   ├── explosion_on_event_device.verse← пример: one-shot VFX по событию
    │   └── physics_launcher.verse     ← пример: физ-импульс (запуск пропа + отброс)
